...
static void write_sector(uint32_t sector, uint8_t *buf) {
  ...
  if (needs_erase) {
    ...
    PY25Q16_WriteBuffer(block_addr, erase_block_buffer, FLASH_ERASE_SIZE, false);
  } else {
...
static int find_file_entry(const char *formatted_name, fat_dir_entry_t *entry) {
  ...
  for (uint32_t sector = ROOT_START_SECTOR; sector < DATA_START_SECTOR; sector++) {
    ...
    for (int i = 0; i < (SECTOR_SIZE / 32); i++) {
      ...
      if (e->name[0] == 0) return -1;  // Конец директории
...
static int update_file_entry(const char *formatted_name, const fat_dir_entry_t *entry) {
  uint8_t buf[SECTOR_SIZE];
...
int usb_fs_write_file(const char *name, const uint8_t *data, uint32_t size, bool append) {
  ...
  while (written < size) {
    uint32_t flash_addr = FLASH_DATA_OFFSET + (cluster - 2) * CLUSTER_SIZE + offset_in_cluster;
...
size_t usb_fs_read_bytes(usb_fs_handle_t *handle, uint8_t *buf, size_t len) {
  ...
  while (len > 0 && handle->position < handle->file_size) {
    ...
    uint32_t remaining_in_cluster = CLUSTER_SIZE - handle->current_position_in_cluster;
    
    if (remaining_in_cluster == 0) {
      uint16_t next_cluster = read_fat_entry(handle->current_cluster);
      if (next_cluster < 2 || next_cluster >= 0xFFF8) break;
...
int usb_fs_list_files(file_info_t *list, int max_count) {
  ...
  for (uint32_t sector = ROOT_START_SECTOR; 
       sector < DATA_START_SECTOR && count < max_count; 
       sector++) {
    uint32_t flash_addr = sector * SECTOR_SIZE;
    PY25Q16_ReadBuffer(flash_addr, buf, SECTOR_SIZE);
...
int usb_fs_get_file_count(void) {
  ...
  for (uint32_t sector = ROOT_START_SECTOR; sector < DATA_START_SECTOR; sector++) {
    ...
    for (int i = 0; i < (SECTOR_SIZE / 32); i++) {
      ...
      if (e->name[0] == 0) return count;
      if (e->name[0] != 0xE5 && !(e->attr & 0x08)) count++;
    }
  }
...
void check_fat_consistency(void) {
  ...
  for (int i = 0; i < SECTORS_PER_FAT; i++) {
    ...
    uint32_t fat2_addr = FLASH_FAT_OFFSET + SECTORS_PER_FAT * SECTOR_SIZE + i * SECTOR_SIZE;
