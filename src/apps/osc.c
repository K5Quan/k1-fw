...
bool OSC_key(KEY_Code_t key, Key_State_t state) {
  ...
  case KEY_2: setScaleV(osc.scale_v + 1, 0); return true;
  case KEY_8: setScaleV(osc.scale_v - 1, 0); return true;

  // Прореживание (скорость развёртки / разрешение FFT по времени)
  case KEY_1: setScaleT(osc.scale_t + 1, 0); return true;
  case KEY_7: setScaleT(osc.scale_t - 1, 0); return true;

  // Уровень триггера (шаг 128 = ~3% шкалы)
  case KEY_3: setTriggerLevel(osc.trigger_level + 128, 0); return true;
  case KEY_9: setTriggerLevel(osc.trigger_level - 128, 0); return true;
  ...
  // Сетка
  case KEY_F: osc.show_grid = !osc.show_grid; return true;
...
static void drawSpectrum(void) {
  if (!osc.fft_fresh) {
    PrintSmallEx(LCD_XCENTER, OSC_TOP_MARGIN + OSC_GRAPH_H / 2,
                 POS_C, C_FILL, "FFT...");
    return;
  }
  ...
  for (int k = 0; k < FFT_BINS; k++) {
    ...
    if (h > (uint32_t)(OSC_GRAPH_H - 1)) h = OSC_GRAPH_H - 1;
    int x0    = k * 2;
...
static void drawStatus(void) {
  ...
  if (osc.mode == MODE_WAVE) {
    PrintSmallEx(LCD_WIDTH, SMALL_FONT_H * 3, POS_R, C_FILL,
                 "V:%d T:%d", osc.scale_v, osc.scale_t);
  } else {
    PrintSmallEx(LCD_WIDTH, SMALL_FONT_H * 3, POS_R, C_FILL,
                 "T:%d", osc.scale_t);
  }
  ...
  PrintSmallEx(0, SMALL_FONT_H * 4, POS_L, C_FILL,
               "Bat:%u", osc.battery_raw);
  if (osc.mode == MODE_WAVE) {
    PrintSmallEx(LCD_WIDTH, SMALL_FONT_H * 4, POS_R, C_FILL,
                 "T:%d", osc.trigger_level);
  }
}
